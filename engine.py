...
def merge_data(bitcoin_file, tweet_file):
    ...
    df_btc.columns = ["Price", "24h_volume_usd", "market_cap_usd", "available_supply", "total_supply",
                      "percent_change_1h", "percent_change_24h", "percent_change_7d", "sell", "buy", "15m", "Time"]
...
def predict(model, scaler):
    ...
    dsn_pwd = "victor"  # e.g. "Password123"
    ...
    while True:
        ...
        input_string = "INSERT INTO live_data values ({},{},{},'{}','{}');".format(yhat_inverse[0][0], bit_data[0][0],
                                                                                   sent_data[4][0],
                                                                                   datetime.datetime.now(
                                                                                       tz=france).strftime(
                                                                                       '%Y-%m-%d %H:%M:%S'), decision)
        print(input_string)
